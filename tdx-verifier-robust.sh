...
# Robust TDX Quote Verifier Script
# This script verifies TDX quotes and attestation tokens with timeout protection
...
# Verify TDX quote file (improved to handle different formats)
verify_quote() {
    local quote_file="$1"
    local quiet_mode="${2:-false}"
    ...
    if [[ ${file_size} -gt 0 ]]; then
        ...
        # Check file type
        local file_type=$(file "${quote_file}" 2>/dev/null || echo "unknown")
        
        if echo "${file_type}" | grep -q "ASCII text"; then
            # Handle text-based quote files (base64 encoded or JSON)
            if [[ "${quiet_mode}" != "true" ]]; then
                log_info "Quote file appears to be text-based"
            fi
            
            # Check if it's base64 encoded
            if head -c 100 "${quote_file}" | base64 -d > /dev/null 2>&1; then
                if [[ "${quiet_mode}" != "true" ]]; then
                    log_success "Quote file appears to be base64 encoded"
                fi
                is_valid=true
            else
                # Check if it's JSON
                if jq empty "${quote_file}" 2>/dev/null; then
                    if [[ "${quiet_mode}" != "true" ]]; then
                        log_success "Quote file appears to be JSON format"
                    fi
                    is_valid=true
                else
                    if [[ "${quiet_mode}" != "true" ]]; then
                        log_warning "Quote file is text but not base64 or JSON"
                    fi
                    is_valid=true  # Still consider it valid if it has content
                fi
            fi
            
        elif echo "${file_type}" | grep -q "data\|binary"; then
            # Handle binary quote files
            if [[ "${quiet_mode}" != "true" ]]; then
            ...
            if [[ "${quiet_mode}" != "true" ]]; then
                log_warning "Quote file format is unknown, but has content"
            fi
            is_valid=true
        fi
...
# Verify all available files (with timeout protection)
verify_all() {
